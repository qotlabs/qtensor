...
ax.bar(list_ranks + 0.125, res_error_ort, width=0.25, lw=3, alpha=0.5, color='blue', label='Ort')
...
plt.ylabel(r'$|E_{\chi} - E|_{\operatorname{av}}$', fontsize=22)
plt.show()
...
plt.scatter(list_ranks, results_mean_ort[0], lw=5, alpha=1, color='blue', label=r'$N = 50, D = 5$' + ' ort')
plt.plot(list_ranks, results_mean_ort[0], lw=5, alpha=0.3, color='blue')
plt.plot(list_ranks, results_mean_exact[0], '--', lw=5, alpha=0.3, color='blue', label='Theory value')
...
# plt.ylim(0, 1.5)
plt.xlabel(r'$\chi$', fontsize=22)
plt.ylabel(r'$E_{\chi}$', fontsize=22)
plt.show()
